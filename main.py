...
async def grab_links(q: asyncio.Queue, url=START_URL) -> asyncio.Queue:
...
async def download_file(url: str, session: ClientSession) -> (str, dt.datetime):
...
async def retrieve_requested_data_from_file(url: str, session: ClientSession) -> pd.DataFrame:
...
async def main(n=10):
    """
    Process inserting data to database.
    n - number of tasks
    """
